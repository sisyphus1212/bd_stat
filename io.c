...
main (int argc, char **argv)
{
	...
	mfd = open("/dev/mem", (memread ? O_RDONLY : O_RDWR) | O_SYNC);//打开相应的硬件，得到文件描述符
	...
	real_io = mmap(NULL, real_len,
			...
			MAP_SHARED, mfd, real_addr);//根据物理地址和文件描述符得到对应的虚拟地址
	if (real_io == (void *)(-1)) {
	...
	if (filename && memread) {
		...
		int n = read(ffd, real_io + offset, req_len);//从文件ffd中读取数据到real_io + offset中
